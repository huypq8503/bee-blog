...
include './navbar.php';
$id = $_GET["id"];
$query = "SELECT category.categoryName, post.id, post.content, post.thumbnail, post.title, post.sub_title, post.categoryID, post.userID FROM post JOIN category on post.categoryID = category.id where categoryID = $id";
$post = getAll($query);
$query2 = "SELECT category.categoryName FROM post JOIN category on post.categoryID = category.id where categoryID = $id";
$h1 = getOne($query2);
?>
...
<body>
    <div class="container">
        ...
        <h1 style="text-align: center; margin-top: 50px;"><?php echo $h1['categoryName'] ?></h1>
        <!-- blog section -->
